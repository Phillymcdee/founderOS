            ...
            {Object.entries(columns).map(([state, stateIdeas]) => (
              ...
              >
                ...
                {stateIdeas.length === 0 ? (
                  ...
                  stateIdeas.map((idea) => (
                    <IdeaCard key={idea.id} idea={idea} signalMap={signalMap} />
                  ))
                )}
              </div>
            ))}
...
function IdeaCard({
  ...
  {idea.autoGenerated && (
    <p style={{ color: '#10b981', fontWeight: 600 }}>
      Auto-generated from recent signals — review before scoring.
    </p>
  )}
      {idea.transformation && (
        <p>
      ...
      {(idea.sourceSignalIds ?? []).length > 0 && (
        <div style={{ marginTop: '1rem' }}>
          ...
          <ul>
            {(idea.sourceSignalIds ?? []).map((signalId) => {
              const signal = signalMap[signalId];
              if (!signal) return null;
              return (
                <li key={signalId}>
                  <strong>{signal.source}:</strong> {signal.content}
                </li>
              );
            })}
          </ul>
